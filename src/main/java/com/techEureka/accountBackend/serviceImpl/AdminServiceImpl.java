...
//		usersInfo.setPassword(encoder.encode(request.getPassword()));
	...
	public BaseResponse updateUserInfo(UpdateUserInfoModel request) {
		...
		try {
			usersInfoRepo.updateUsersInfo(request.getUserId(), request.getUserName(), request.getEmail(), request.getPhone(), request.getPassword(), request.getName(), request.getDob(), request.getBlood(), request.getAddress());
