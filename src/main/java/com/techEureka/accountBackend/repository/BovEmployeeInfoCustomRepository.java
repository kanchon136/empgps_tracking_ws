...
public class BovEmployeeInfoCustomRepository {
	...
	public List<BovEmployeeInfo> findAllBovEmployeeInfos(){
		String query = "SELECT MKGPROF_NO,EMPLOYE_NO,EMPLOYE_NM,DESIG_CODE,PTGPROF_NO,ACTVSTS_FG"
				+ " FROM BOV_EMPLYEE";
		log.info("query======:"+query);
		List<BovEmployeeInfo> bovEmployeeInfos = jdbcTemplate.query(query, (rs,rownum) -> {
			
			BovEmployeeInfo bovEmployeeInfo = new BovEmployeeInfo(rs.getString("MKGPROF_NO"), rs.getString("EMPLOYE_NO"),
					rs.getString("EMPLOYE_NM"), rs.getString("DESIG_CODE"), rs.getString("PTGPROF_NO"),rs.getInt("ACTVSTS_FG"));
